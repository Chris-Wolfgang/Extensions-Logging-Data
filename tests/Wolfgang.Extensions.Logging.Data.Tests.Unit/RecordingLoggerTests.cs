...
public class RecordingLoggerTests
{
    ...
    [Fact]
    public void Log_records_level_eventId_message_exception_and_structured_values()
    {
        var logger = new RecordingLogger();
        ...
        logger.Log(LogLevel.Error, new EventId(7, "Seven"), state, exception, (_, _) => "formatted");
        ...
        Assert.True(entry.ContainsKey("Null"));
        Assert.False(entry.ContainsKey("Missing"));
    }
    ...
    public void Log_when_state_is_not_a_structured_value_list_records_null_values()
    {
        ...
        Assert.False(entry.ContainsKey("anything"));
    }
}
